...
export class HTMLDesignComponent implements OnInit, AfterViewInit {
  ...
  /** 页面加载完成 */
  @Output() onLoaded: EventEmitter<boolean> = new EventEmitter(false);
  /** 页面开始加载 */
  @Output() onStartLoad: EventEmitter<string> = new EventEmitter();
  /** 页面改变 */
  @Output() onPageChanged: EventEmitter<string> = new EventEmitter();
  ...
  private onUrlChanged() {
    this.onPageChanged.emit(this._url);
    //在切换页面时重新初始化
    this.init();
  }
  ...
  private init() {
    ...
    //加载完JS，并执行JS的初始化后
    this.iframeChatService.onScriptInited.subscribe((x: string) => {
      this.getIframePos();
      forkJoin([
    ...
    //先加载JS
    this.iframeChatService.onConnected.subscribe((x: boolean) => {
      //优先加载JS
    ...
    this.iframeChatService.onTrackHoverElement.subscribe((info: ElementInfo[]) => {
      ...
      info.forEach((theInfo) => {
        let theAddon = this.addons.find((x) => x.Name.toLocaleLowerCase() == theInfo.type);
    ...
    this.iframeChatService.onScroll.subscribe((y: number) => {
      this.hoverBox.quit();
    });
    this.iframeChatService.onsizechanged.subscribe((height: number) => {
      this.hoverBox.quit();
