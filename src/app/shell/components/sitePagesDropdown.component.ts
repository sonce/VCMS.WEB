...
@Component({
	...
	template: `
				...
				<ng-template #treeNodeWrapperTemplate let-node="node" let-index="index">
					<ng-container *ngIf="convetToTreeNode(node) as toTreeNode">
						<div class="node-wrapper align-items-center" [style.padding-left]="toTreeNode.getNodePadding()">
							...
							<div
								class="node-content-wrapper d-flex align-items-center"
								...
								[treeAllowDrop]="toTreeNode.allowDrop"
								[treeDrag]="node"
								[treeDragEnabled]="toTreeNode.allowDrag()"
							>
								<ng-container *ngIf="convertToSitePage(toTreeNode.data) as toSitePage">
									<form
										*ngIf="Editable && currentChangeNamePageId == toSitePage.id"
										class="w-100 needs-validation"
										(ngSubmit)="saveName(toSitePage)"
										[formGroup]="changeNameForm"
										novalidate
									>
										<div class="input-group">
											<input
												[ngModel]="toSitePage.name"
											...
											<span class="input-group-btn">
												<button
													type="button"
													type="submit"
													[disabled]="changeNameForm.invalid"
													click-stop-propagation
									...
									<ng-container *ngIf="currentChangeNamePageId != toSitePage.id">
										<div>
											...
											<i
												...
												[class.icon-page]="toSitePage.PageType == SitePageType.Page"
											></i>
										</div>
										<div>
										...
										<div class="ml-auto" *ngIf="Editable">
											<i
												...
												[class.icon-ico-hidden]="toSitePage.Hidden === true"
											></i>
											<button
												class="btn btn-default context-menu-button btn-circle text-primary"
												placement="right"
												container="body"
												...
												#p="ngbPopover"
												click-stop-propagation
												(click)="openContextMenu(p, toSitePage)"
												(shown)="toggleContextMenu(p)"
												(hidden)="toggleContextMenu(p)"
											>
		...
		<ng-template #popContent let-sitePage="sitePage">
			<app-multilevelnav
				*ngIf="convertToSitePage(sitePage) as toSitePage"
				[isVertical]="true"
				[noBorder]="false"
				[item]="toSitePage"
			>
				...
				<ng-template
					multiLevelNavItem
					(execute)="setToHomePage(toSitePage)"
					[visible]="!toSitePage.IsHomePage && toSitePage.PageType == SitePageType.Page"
				>
				...
				<ng-template
					multiLevelNavItem
					[divider]="true"
					[visible]="levelUPVisible(sitePage) || levelDownVisible(sitePage)"
				></ng-template>
...
export class SitePagesDropdownComponent implements OnInit {
	...
	dropdownpagesLoader = 'dropdownpagesLoader';
	changePagenameLoader = 'changePagenameLoader';
	...
	options: ITreeOptions = {
		allowDrag: (node: TreeNode) => {
			if (this.Editable) {
				if (_.isNil(this.currentChangeNamePageId)) return true;
		...
		allowDrop: (element: TreeNode, t: { parent: TreeNode; index: number }) => {
			if (element.id != t.parent.id) {
				...
				if (t.parent.data instanceof SitePage) {
					const toData = t.parent.data as SitePage;
					if (toData.ParentId != '0') return false;
				}
				return true;
			} else return false;
		},
		actionMapping: {
			mouse: {
				drop: (
					...
					setTimeout(() => {
						if (info.from.parent == info.to.parent && info.from.index < info.to.index) info.to.index -= 1;

						const currentPage = info.from.data as SitePage;
						const oldParent = info.from.parent.data as SitePage;
						const newParent = info.to.parent.data as SitePage;
				...
				dblClick: (tree: TreeModel, node: TreeNode): void => {
					if (this.Editable) {
				...
				click: (tree: TreeModel, node: TreeNode, $event: MouseEvent): void => {
					this.timer = window.setTimeout(
						function (this: SitePagesDropdownComponent) {
							if (!this.prevent) {
								$event.shiftKey
	...
	onPageActive(event: { eventName: string; node: TreeNode; treeModel: TreeModel }): void {
	...
	openContextMenu(popover: NgbPopover, toSitePage: SitePage): void {
		if (popover.isOpen()) {
			popover.close();
		} else {
	...
	/** 是否已经的打开菜单 */
	toggleContextMenu(p: NgbPopover): void {
		this.isOpenContextMenu = p.isOpen();
	}

	setToHomePage(sitePage: SitePage): void {
	 ...
	 * 删除页面
	 */
	delPage(sitePage: SitePage): void {
		this.spinner.show(this.changePagenameLoader);
		setTimeout(() => {
			let parentChilds;
			if (_.isNil(sitePage.ParentPage)) parentChilds = this.nodes;
			else parentChilds = sitePage.ParentPage.children;
			const delIndex = parentChilds.indexOf(sitePage);
	 ...
	 * 修改页面是否可见
	 */
	toggleVisible(sitePage: SitePage): void {
		this.spinner.show(this.dropdownpagesLoader);
		setTimeout(() => {
	...
	/** 当前正在编辑的名字 */
	setInChangeNameMode(value: { event: MouseEvent | Event; item: SitePage }): void {
	...
	/** 修改名字LOADING */
	saveName(toTreeNode: SitePage): void {
	...
	private onMouseOut() {
		if (this.vdropdown.IsOpen && !this.isOpenContextMenu) this.vdropdown.close();
	}

	//#region 菜单的操作
	levelUPVisible(data: SitePage): boolean {
		return false;
		//顶级，不能往上
		if (_.isNil(data.ParentPage)) return false;
		return true;
	}
	levelDownVisible(data: SitePage): boolean {
		return false;
		//在第二级，最多2级，所以不能往下
		if (!_.isNil(data.ParentPage)) return false;
		//有子页面，不能往下
		if (!_.isNil(data.children) && data.children.length > 0) return false;
		return true;
	}
	//#endregion

	//#region 数据类型转换
	convetToTreeNode = (fromObj: unknown): TreeNode => fromObj as TreeNode;
	convertToSitePage = (fromObj: unknown): SitePage => fromObj as SitePage;
	//#endregion
}
